...
export default () => {
  ...
  const { filtros, sensoresData, sensores, sensoresOpciones } =
  ...
  const crearSensor = async () => {
    if (!sensor.value.clave)
      return notificacion.nAviso({
        mensaje:
          "No puede crear un sensor, sin asignarle un clave de identificacion.",
      });
  ...
  onMounted(() => {
    ...
    sensoresStore.obtenerSensores().then(console.log).catch(console.log);
  });
