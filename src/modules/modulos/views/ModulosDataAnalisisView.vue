        ...
        <div class="grid grid-cols-12">
            ...
            <section class="p-5 col-span-full rounded-md" :class="{ 'bg-slate-900': hayElementos }">
                <template v-if="(hayElementos || hayDatosFechas)">
                    <div class="p-5 w-full bg-slate-200 rounded-md">
                        <template v-if="hayElementos">
                            <BarChart :="barChartProps" />
                        </template>
                        <template v-else>
                            <LineChart v-if="hayElementosExistentes" :="lineChartProps" />
...
const asignarDataSensores = (data) => {
    const dataTransformada = data.map(({ value }) => value?.data[0]);

    if(!dataTransformada.every(datos => !!datos)) 
        return;
...
const etiquetaTiempoSensoresFecha = computed(() => listadoDataFechas.value?.[0]?.map(({ creado }) => formatearHora(creado)) ?? []);
const dataSensoresFecha = computed(() => {
    if (!hayElementosExistentes.value) return [];

    return listadoDataFechas.value.map(data => {
